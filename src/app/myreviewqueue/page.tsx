...
const mockReviewItems = [
  {
    id: "1",
    type: "Email Draft",
    typeColor: "blue" as TagColor,
    preview: "Subject: Q2 Client Update\nHi John, just wanted to follow up...",
    context: "Drafted based on similar client inquiries.",
  },
  {
    id: "2",
    type: "Meeting Reschedule",
    typeColor: "yellow" as TagColor,
    preview: "Reschedule: Project Sync to Friday 2pm.",
    context: "Proposed to optimize your focus time.",
  },
  {
    id: "3",
    type: "Task Suggestion",
    typeColor: "green" as TagColor,
    preview: "Automate weekly report generation.",
    context: "Suggested based on your recurring tasks.",
  },
  {
    id: "4",
    type: "Important Notification",
    typeColor: "red" as TagColor,
    preview: "Your approval is needed for expense #1234.",
    context: "Flagged as urgent by finance workflow.",
  },
];

type TagColor = 'gray' | 'blue' | 'green' | 'red' | 'yellow' | 'purple' | 'pink' | 'indigo';

export default function MyReviewQueuePage() {
  const { isAuthenticated, isLoading, requireAuth, user, session, error } = useAuth();
    ...
    <>
      ...
      <Container>
        <div className="flex items-center mb-8">
          ...
          <Badge variant="primary" size="md">{items.length} pending actions</Badge>
        </div>
        <div className="space-y-6">
          {items.length === 0 ? (
            <div className="text-center text-[var(--text-secondary)] py-16 text-lg">All caught up! 🎉</div>
          ) : (
            items.map((item) => (
              <Card key={item.id} variant="elevated" padding="md" className="">
                <CardHeader className="flex items-center gap-3 pb-2">
                  <Tag color={item.typeColor} size="sm" variant="solid">
                    {item.type}
                  </Tag>
                  <span className="text-sm text-[var(--text-secondary)] ml-2">{item.context}</span>
                </CardHeader>
                <CardBody className="py-2">
                  <pre className="whitespace-pre-wrap text-[var(--text-main)] text-base font-medium bg-transparent p-0 m-0 border-0">{item.preview}</pre>
                </CardBody>
                <CardFooter className="flex items-center justify-between pt-2 gap-2 flex-wrap">
                  <div className="flex gap-2">
                    <Button variant="primary" size="sm">Approve & Execute</Button>
                    <Button variant="secondary" size="sm">Edit</Button>
                    <Button variant="danger" size="sm" onClick={() => handleDismiss(item.id)}>Dismiss</Button>
                  </div>
                  <div className="flex items-center gap-1 ml-auto">
                    <span className="text-xs text-[var(--text-secondary)] mr-1">Was this helpful?</span>
                    <Tooltip content="Thumbs up" position="top">
                      ...
                      >
                        <svg 
                          className={`w-5 h-5 ${feedback[item.id] === 'up' ? 'text-blue-500' : 'text-[var(--text-secondary)]'}`}
                          fill="currentColor" 
                          stroke="currentColor" 
                          viewBox="0 0 20 20"
                        >
                    ...
                    <Tooltip content="Thumbs down" position="top">
                      <Button
                        variant="transparent"
                        size="icon"
                      ...
                      >
                        <svg 
                          className={`w-5 h-5 ${feedback[item.id] === 'down' ? 'text-blue-500' : 'text-[var(--text-secondary)]'}`}
                          fill="currentColor" 
                          stroke="currentColor" 
                          viewBox="0 0 20 20"
                        >
                        ...
                        <svg className={`w-5 h-5 ${feedback[item.id] === 'down' ? 'text-blue-500' : 'text-[var(--text-secondary)]'}`} fill="currentColor" stroke="currentColor" viewBox="0 0 20 20">
                          <path d="M9.50881 17.7486C9.33158 18.0537 8.97577 18.213 8.6255 18.1441C7.03327 17.8308 6.06502 16.2324 6.54255 14.7055L7.24966 12.322H5.92157C3.73272 12.322 2.14302 10.2743 2.71895 8.1967L3.85063 4.23665C4.2445 2.81575 5.55627 1.83 7.05325 1.83L12.0305 1.83C12.9457 1.83174 13.687 2.56219 13.687 3.463V12.322H13.0711C12.7732 12.322 12.4983 12.6015 12.3505 12.8559L9.50881 17.7486Z" />
                          <path d="M15.0125 12.322C16.3875 12.322 17.3346 11.1825 17.3346 9.8297V4.23665C17.3346 2.88384 16.3875 1.83 15.0125 1.83H14.5684C14.8508 2.31039 15.0125 2.86812 15.0125 3.463V12.322Z"></path>
                        </svg>
                      </Button>
                    </Tooltip>
                  </div>
